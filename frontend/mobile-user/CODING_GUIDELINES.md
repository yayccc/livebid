...
- API 返回值、表单值、WebSocket 事件都必须有明确类型。
...
- 移动端优先设计，最小宽度按 320px 处理。
- 直播间、商品卡片、竞拍按钮、倒计时、价格变化等核心交互应独立成组件。
- 实时数据与普通接口数据分开封装，避免 WebSocket 逻辑散落在组件里。
- 出价、倒计时、支付等关键流程必须显式处理 loading、disabled、error 和重试状态。
